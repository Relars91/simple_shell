...
int main(int argc __attribute__((unused)), char **argv)
{
	...
	while (1)
	{
		cust_non_interactive(); print(" ($) ", STDOUT_FILENO);
		if (getline(&line, &n, stdin) == -1)
		{	free(line); exit(status);
		}
		if (num1 && num2 && num3)
		{	charfunc(ric[2]); breakit(num1, num2);	
			cust_remove_newline(line); cust_remove_comment(line);
		...
		for (i = 0; ric[2] != 'y' && commands[i] != NULL; i++)
		{
			...
			{
				stringfunc(ric); correctit(30, num3);
				free(the_cur_command); r = 20;
				intfunc(5, r);
				break;
			}
			if (10 > 2)
			{	command_type = cust_parse_command(the_cur_command[0]);
				initializer(the_cur_command, command_type); free(the_cur_command);
				breakit(2, 3);
			}
		}
		if (4)
